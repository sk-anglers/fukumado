...
import { twitchEventSubWebhookService } from './twitchEventSubWebhookService';
import { priorityManager } from './priorityManager';
import type { PriorityChangeEvent } from '../types/priority';

/**
 * チャンネル割り当て方式
 */
type AllocationMethod = 'eventsub' | 'webhook' | 'polling';
 ...
 */
export class DynamicChannelAllocator {
  // チャンネルID → 割り当て情報
  private allocations: Map<string, ChannelAllocation> = new Map();

  // EventSub容量の閾値（90%で警告）
  private readonly capacityWarningThreshold = 0.9;
  ...
  private twitchClientId: string | null = null;
   ...
   * Twitch認証情報を設定
   */
  public setTwitchCredentials(accessToken: string, clientId: string): void {
   ...
   * Webhook設定を有効化
   */
  public enableWebhook(webhookUrl: string, webhookSecret: string): void {
   ...
   * 優先度変更イベントを処理
   */
  private async handlePriorityChange(event: PriorityChangeEvent): Promise<void> {
   ...
   */
  private async allocateRealtimeChannels(channelKeys: string[]): Promise<void> {
   ...
   * リアルタイムチャンネルの割り当てを解除
   */
  private async deallocateRealtimeChannels(channelKeys: string[]): Promise<void> {
   ...
   * 割り当て方式を決定
   */
  private async determineAllocationMethod(): Promise<AllocationMethod> {
    // EventSubの容量を確認
   ...
   * EventSubに割り当て
   */
  private async allocateToEventSub(channelId: string, platform: 'youtube' | 'twitch'): Promise<void> {
    console.log(`[DynamicChannelAllocator] Allocating ${platform}:${channelId} to EventSub`);

    // EventSubManagerにサブスクライブ
    await twitchEventSubManager.subscribeToUsers([channelId]);
   ...
   * EventSubから割り当て解除
   */
  private async deallocateFromEventSub(channelId: string, platform: 'youtube' | 'twitch'): Promise<void> {
    console.log(`[DynamicChannelAllocator] Deallocating ${platform}:${channelId} from EventSub`);

    // EventSubManagerからアンサブスクライブ
    await twitchEventSubManager.unsubscribeFromUsers([channelId]);
   ...
   * Webhookに割り当て
   */
  private async allocateToWebhook(channelId: string, platform: 'youtube' | 'twitch'): Promise<void> {
    console.log(`[DynamicChannelAllocator] Allocating ${platform}:${channelId} to Webhook`);

    // Webhookサービスにサブスクライブ
    await twitchEventSubWebhookService.subscribeToUsers([channelId]);
   ...
   * Webhookから割り当て解除
   */
  private async deallocateFromWebhook(channelId: string, platform: 'youtube' | 'twitch'): Promise<void> {
    console.log(`[DynamicChannelAllocator] Deallocating ${platform}:${channelId} from Webhook`);

    // Webhookサービスからアンサブスクライブ
    await twitchEventSubWebhookService.unsubscribeFromUsers([channelId]);
   ...
   * チャンネルキーをパース
   */
  private parseChannelKey(key: string): ['youtube' | 'twitch', string] {
    const [platform, channelId] = key.split(':');
    return [platform as 'youtube' | 'twitch', channelId];
  }
  ...
  public getStats(): AllocationStats {
    ...
    const eventSubCapacity = twitchEventSubManager.getCapacity();
   ...
   * 割り当て状況をログ出力
   */
  private logAllocationStats(): void {
   ...
   * 特定のチャンネルの割り当て情報を取得
   */
  public getAllocation(channelId: string, platform: 'youtube' | 'twitch'): ChannelAllocation | null {
    const key = `${platform}:${channelId}`;
...
// シングルトンインスタンス
export const dynamicChannelAllocator = new DynamicChannelAllocator();
