...
exports.apiLogStore = void 0;
class ApiLogStore {
    constructor() {
        this.logs = [];
        this.MAX_LOGS = 1000; // メモリ制限
    }
    /**
     * APIログを追加
     */
     ...
     */
    getTodayYouTubeQuotaUsage() {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const todayTimestamp = today.getTime();
        const youtubeLogs = this.logs.filter(log => log.service === 'youtube' &&
            log.quotaCost &&
     ...
     * 直近N分間のAPI呼び出し数を取得
     */
    getRecentCallCount(service, minutes) {
        const threshold = Date.now() - (minutes * 60 * 1000);
        return this.logs.filter(log => log.service === service &&
            new Date(log.timestamp).getTime() >= threshold).length;
    }
    /**
     * ログをクリア
     */
...
// シングルトンインスタンス
exports.apiLogStore = new ApiLogStore();
//# sourceMappingURL=apiLogStore.js.map