...
const twitchConduitManager_1 = require("./twitchConduitManager");
...
class TwitchEventSubManager {
    ...
    async connectAll() {
        ...
        if (this.mode === 'websocket') {
            ...
            await twitchConduitManager_1.twitchConduitManager.initialize();
            // 初期シャードを作成（1つ）
            await twitchConduitManager_1.twitchConduitManager.createWebSocketShard('0');
    ...
    async subscribeToUsers(userIds) {
        ...
        if (this.mode === 'websocket') {
            ...
            await twitchConduitManager_1.twitchConduitManager.subscribeToUsers(userIds);
        }
    }
    /**
     * チャンネルのサブスクリプションを解除
    ...
    async unsubscribeFromUsers(userIds) {
        ...
        if (this.mode === 'websocket') {
            ...
            // Conduitsモード: ConduitManagerに委譲
            await twitchConduitManager_1.twitchConduitManager.unsubscribeFromUsers(userIds);
        }
    }
    /**
     * イベントハンドラーを登録
    ...
    disconnectAll() {
        ...
        if (this.mode === 'websocket') {
            ...
            twitchConduitManager_1.twitchConduitManager.disconnect();
    ...
    isSubscribed(userId) {
        if (this.mode === 'websocket') {
            ...
            return twitchConduitManager_1.twitchConduitManager.isSubscribed(userId);
        }
    }
    /**
    ...
    getSubscribedUserIds() {
        if (this.mode === 'websocket') {
            ...
            return twitchConduitManager_1.twitchConduitManager.getSubscribedUserIds();
        }
    }
    /**
     * 統計情報を取得
    ...
    async getStats() {
        if (this.mode === 'websocket') {
            ...
            const conduitStats = await twitchConduitManager_1.twitchConduitManager.getStats();
            return {
                mode: this.mode,
                totalConnections: 1, // Conduit自体
                activeConnections: 1,
                totalSubscriptions: conduitStats.totalSubscriptions,
                subscribedChannelCount: twitchConduitManager_1.twitchConduitManager.getSubscribedUserIds().length,
...
exports.twitchEventSubManager = new TwitchEventSubManager(eventSubMode);
