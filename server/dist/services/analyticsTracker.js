...
class AnalyticsTracker {
    constructor(redisClient) {
        this.RETENTION_DAYS = 90; // データ保持期間（日）
     ...
     * イベントタイプ別の集計処理
     */
    async aggregateEvent(event, dateKey) {
        ...
        switch (event.type) {
            ...
            case 'feature_use': {
                ...
                if (platform) {
                    promises.push(this.redis.hincrby(`analytics:platforms`, platform, 1), this.redis.hincrby(`analytics:platforms:daily:${dateKey}`, platform, 1), this.redis.expire(`analytics:platforms:daily:${dateKey}`, ttl));
                }
            ...
            case 'session_end': {
                ...
                if (event.type === 'session_end' && event.data.duration) {
                    // セッション時間を記録
                    await this.redis.lpush(`analytics:sessions:durations:${dateKey}`, event.data.duration.toString());
    ...
    async getStats(days = 30) {
        try {
            const [totalEvents, layoutSlots, layoutPreset, devices, screenSizes, buttons, features, platforms, streamActions, streamPlatforms, authLogins, authLogouts, dailyStats] = await Promise.all([
     ...
     * 平均セッション時間を計算
     */
    async getAverageSessionDuration(days) {
        const now = new Date();
        let totalDuration = 0;
        let count = 0;
        for (let i = 0; i < days; i++) {
